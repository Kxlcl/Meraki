...
app.get('/businesses/:id', async (req, res) => {
  ...
  if (!mongoose.Types.ObjectId.isValid(id)) {
    ...
    return res
      .status(400)
      .render('error', { message: 'Invalid Business ID format', error: null });
  }
  ...
  try {
    ...
    if (!business) {
      ...
      return res
        .status(404)
        .render('error', { message: 'Business not found', error: null });
    }
    ...
    res
      .status(500)
      .render('error', { message: 'Database error occurred', error: err });
  }
});
